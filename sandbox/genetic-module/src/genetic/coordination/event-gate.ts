...
import type { tGeneticModule } from '../resource/genetic-module'

export function createGeneticEventGate(deps: {genetic: tGeneticModule}) {
    ...
    function establishBaseline(files: tGeneticFileSnapshot[]) {
        ...
        deps.genetic.control.establishBaseline(normalized)
    ...
    function flush(): tGeneticEventGateFlush {
        requireBaseline()
        const pendingAction = deps.genetic.debug.pendingAction()
        if (pendingAction) {
        ...
        const result = deps.genetic.control.scan(toFiles(nextFiles))

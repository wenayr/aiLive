...
test('manual patch capture turns real saved edits and deletions into gate events', async () => {
    ...
    try {
        ...
        const initialNormalizer = simulation.debug.snapshot().files.find(function isNormalizer(file) {
        ...
        assert.deepEqual(patched.notification.acceptedPaths, ['adapter.ts'])
        ...
        assert.deepEqual(luna.pendingAction?.kind == 'inspect' ? luna.pendingAction.files : [], [{
            path: 'normalizer.ts',
            beforeRevision: initialNormalizer.revision,
            afterRevision: null,
        }])
    } finally {
